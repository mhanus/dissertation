...
\begin{equation*}
  ...
  \right.
\end{equation*}
has a unique weak solution $\psi(\br,\bomega,E)\in\Dom{T}\subset\Lp[1](X)$ for any $q\in \Lp[1](X)$.
\end{theorem}
\begin{proof}
...
show the existence of unique solution in $\Lp[\infty](X)$ for $q\in \Lp[\infty](X)$; the proof in this case is based on
 the inversion of the transport operator along characteristics (see \ref{sec:advection} above) and requires instead of
 (c) the condition
\begin{enumerate}
...
In general $\Lp(X)$ spaces with $1 < p < \infty$, Dautray and Lions in 
\cite[\S2, Chap. XXI]{DautrayLions} also outline the proof based on the same ideas as those used in the proof of Theorem
\ref{thm1} (theory of monotone operators), utilizing assumptions (a-d) of Thm. \ref{thm1}. The case $p = 2$ is
particularly important\footnote{even though it does not lead to a direct physical interpretation as the $\Lp[1](X)$ setting} as the Hilbert space structure of 
$\Lp[2](X)$ allows to use richer set of mathematical tools to formulate practical solution methods  (like those based on
Fourier transform or the finite element methods). Agoshkov provides in \cite[Chap. 3]{Agoshkov} an
extensive study of this case using variational methods. We will now take yet another approach and prove the
...
\genop{A}u = f
\end{equation}
is rotationally invariant, if $\genop{A}u = f$ implies $\genop{A}Ru = Rf$ for any operator
...
\begin{lemma}\label{lemma1}
	$\genop{A}u = f \ \Rightarrow \genop{A}Ru = Rf\,\ \forall R\in \mathrm{SO}(3)$ if and only if $\genop{A}R = R\genop{A}$.
\end{lemma}
\begin{proof}
	Sufficiency is obvious by operating with $R$ on both sides of eq. \eqref{eq:op1}.
	We will show neccessity indirectly, i.e. we suppose that there exists $R\in\mathrm{SO}(3)$ such that $\genop{A}R \neq R\genop{A}$
	and show that then we can have $\genop{A}u = f$ but $\genop{A}Ru \neq Rf$. Indeed, assuming $\genop{A}u = f$ and operating by $R$, we get  
	$$
		Rf = R\genop{A}u \neq \genop{A}Ru,
	$$
