...
We will introduce the NTE in Chap. \ref{chap:nte-review} as an integro-differential equation in 6 dimensions. This high
...
\myparagraph{The $\MCPN$ approximation and its relation to the $\SPN$ approximation} 
...
harmonics of order $N$} -- which is the subject of the second part of Chap. \ref{chap:MCSH}. So far, the author has only
...
\myparagraph{Finite element framework for 2D neutron diffusion/transport problems}
...
cards available in our project server). Even though the implementation is not yet competitive with the well-optimized
