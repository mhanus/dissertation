...
\sref{sec:SI}.
\comment{
We will illustrate it by demonstrating that the $\SN$ approximation doesn't 
...
\subsubsection{Operator form of the $\SN$ approximation}
...
unique solution of the NTE (see \sref{sec:fixed-source} for examples).
%
\comment{
...
transforms a function $u\in V$ to a spatial vector function $\mathrm{F}$ as:
 \begin{equation}\label{eq:map_SN_inv}
	\mathrm{F}(\br) = \bigl(\PihSN u)(\br) = \col\left\{u(\br,\bomega_m)\right\}_{\idxset{M}}. %\equiv
...
As a final remark, we would like to point out that $\PiSN$ is not an orthogonal projection in $V = \Lp[2](X)$ as it is
...
later in \sref{sec:dif}) tends to the solution of the NTE under such conditions. 
...
($k = {}_n^m$) and the form of spherical harmonics with $n = 0,1$, we also obtain a direct correspondence of the first four moments and the physically important quantities defined in \sref{sec:qoi}:
$$
