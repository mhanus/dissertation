...
\begin{itemize}
  ...
  %also, $\mathbf{A}(\br)$ will be used for matrix fields with elements being functions defined in $\VV$;
...
\subsection{Drawbacks of the $\PN$ approximation}
...
$$
\comment{
Consider now the action of $\mat{K}_{S_N}$ on the $\SN$ representation of $\Y{r}{s}(\bomega)$ for some admissible
index pair $r,s$:
$$
\begin{aligned}
	\mat{K}_{S_N} \bigl(\PihSN \Y{r}{s}\bigr) &= \sum_{n=1}^M w_n
