...
In \cref{chap:mcpn}, we will introduce new symbols for Cartesian tensors.
...
efficient numerical solution. We shall return to convergence properties of this iteration in \sref{sec:SI}; here we
...
\sref{sec:rotinv}) is lost when approximating the continuous NTE by a finite $\SN$ system. 
...
source iteration} (SSI) \index{SSI|see{scattering source iteration}} is utilized, in which the system \eqref{eq:sn1} is
...
solution of the NTE (see \sref{sec:fixed-source} for examples)\comment{, but we will consider specifically  $V =
\Hp[2](\XE)$ to facilitate the comparison with the $\PN$ method}. The spatial vector functions representing the $\SN$ unknowns \eqref{eq:sn_vars} can be obtained from function $f$ by means of the following mapping:
...
discontinuous Galerkin (DG) method \index{DG|see{discontinuous Galerkin method}} with piecewise constant shape functions (DG(0))
...
\begin{remark}[\textsc{Multigroup case}]\label{rem:MGSI}
