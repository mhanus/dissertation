...
\end{multlined} 
$$
where $E^{g+1}+\dEh{g+1} = E^{g}-\dEh{g}$, and equations \eqrefs{eq1}{eq:nte3} are integrated over each energy group
range
...
\cite{Cho2,Wu1,Hursin1,Petkov1,Sanchez1})\index{method!of collision probabilities}\index{method!of characteristics}. As
...
\index{method!of spherical harmonics|(}
...
\index{method!of discrete ordinates|(}
...
\subsubsection{Advection}\label{sec:SN_advection}
...
\index{DG|see{Method: discontinuous Galerkin}} with piecewise constant shape functions generating \linebreak
...
Problem analogous to that described in the previous paragraph also lies behind the restriction to isotropic scattering,
...
intertwining the angular and spatial resolution). An alternative, attractive in particular when an unstructured spatial
...
\index{method!discontinuous Galerkin|(}
...
As before, after restricting to $\Space_{hp} \subset H^1(\VV)$ we get the approximate problem:
...
details.
\comment{ Even in this case, with the simplest angular discretization, the system of linear algebraic equations is --
given the complicated geometry and material arrangement of realistic problems -- very large. In realistic cases, it is
...
of heterogeneous nuclear reactor cores), the long-term experience may be used to create the mesh by hand using
well-established geometry and mesh generation software like the commercial CUBIT (\cite{CUBIT}) or the open-source GMSH
(\cite{GMSH}). When the a-priori knowledge of important solution features is not available, some sort of automatic
adaptivity is needed, which we will discuss in a broader sense (including also the adaptivity in angular domain) in the
following section.
